  ...
  # aquí es el problema
    def filter_products
      ...
      if role == 'store-admin' || role == 'store'
        ...
        @products = Product.includes(:warehouse).where(current: true, shared: true).select(:id, :unique_code, :description, :exterior_color_or_design, :line, :price, :shared, :store_id, :warehouse_id)
      end
      @products
    end
