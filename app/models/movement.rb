...
#      end
  ...
  def vinculate_warehouse
    WarehouseEntry.last.update(movement: self) if self.product_id == WarehouseEntry.last.product_id
  end
          ...
          warehouses.order(:id).each do |entry|
            ...
            if total_quantity >= entry.fix_quantity
              ...
              unless hash_1["subtotal"] == nil
                ...
                actual_discount = ('%.2f' % (discount * q)).to_f
                if product.group
                  actual_subtotal = ('%.2f' % (subtotal * q * hash_1["kg"])).to_f.round(2)
                  actual_discount = ('%.2f' % (discount * q * hash_1["kg"])).to_f.round(2)
                  hash_1["total_cost"] = (cost * q * hash_1["kg"]).round(2)
                end
                hash_1["subtotal"] = actual_subtotal
                hash_1["discount_applied"] = actual_discount
