class Product < ActiveRecord::Base
  ...
  after_save :validate_pieces

  after_save :delete_stores_inventories

  def delete_stores_inventories
    if self.shared == false || current == false
      Store.where.not(store_type: 2).each do |store|
        store_id = store.id
        products = Product.where("shared = true AND current = true AND (classification = 'de línea' OR classification = 'especial') AND child_id is NULL OR store_id = #{store_id}").order(:id).pluck(:id)
  ...
  def validate_pieces
    if self.pieces_per_package == nil || self.pieces_per_package < 1
      self.update(pieces_per_package: 1)
    end
  end
...
#  after_save :create_update_change_table
  ...
  def price_with_discount_stores
    @price = (self.price * (1 - (self.discount_for_stores.to_f / 100))).round(2)
  end

  def price_with_discount_franchises
    @price = (self.price * (1 - (self.discount_for_franchises.to_f / 100))).round(2)
  end
